<template>
    <div class="container mx-auto p-4">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-3xl font-bold text-pink-700">Преподаватели</h1>
            <button 
                v-if="isAdmin" 
                @click="showForm = true" 
                        ...
                        <td class="py-3 px-4">
                            <div class="table-actions flex space-x-2">
                                <button 
                                    ...
                                    class="flex items-center text-sm bg-pink-100 hover:bg-pink-200 text-pink-700 px-3 py-1 rounded-lg transition-all"
                                >
        ...
        <div v-if="showForm" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div class="bg-white p-6 rounded-xl max-w-lg w-full shadow-xl">
                <h2 class="text-xl font-bold mb-4 text-pink-700">{{ form.id ? 'Редактировать преподавателя' : 'Добавить преподавателя' }}</h2>
                <Form @submit="saveTeacher">
                    <div class="grid grid-cols-1 gap-4">
                        ...
                        <input v-model="form.hire_date" type="date" placeholder="Дата найма" class="border border-pink-200 p-2 rounded-lg focus:ring-2 focus:ring-pink-300 focus:border-transparent" required />
                        <input v-model="form.department" type="text" placeholder="Кафедра" class="border border-pink-200 p-2 rounded-lg focus:ring-2 focus:ring-pink-300 focus:border-transparent" required />
                        <input v-model="form.office_location" type="text" placeholder="Место работы" class="border border-pink-200 p-2 rounded-lg focus:ring-2 focus:ring-pink-300 focus:border-transparent" />
                        <input v-model="form.qualification" type="text" placeholder="Квалификация" class="border border-pink-200 p-2 rounded-lg focus:ring-2 focus:ring-pink-300 focus:border-transparent" />
                    ...
                    <div class="mt-4 flex justify-end space-x-2">
                        ...
                        <button @click="showForm = false" class="bg-pink-100 hover:bg-pink-200 text-pink-700 px-4 py-2 rounded-lg transition-colors">
                            Отмена
                        </button>
                    </div>
                </Form>
        ...
        <div v-if="showView" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div class="bg-white p-6 rounded-xl max-w-lg w-full shadow-xl">
                <h2 class="text-xl font-bold mb-4 text-pink-700">Просмотр преподавателя</h2>
                <div class="grid grid-cols-1 gap-3">
                    <p><strong class="text-pink-600">ID:</strong> {{ viewTeacherData.id }}</p>
                    <p><strong class="text-pink-600">Имя:</strong> {{ viewTeacherData.first_name }}</p>
                    <p><strong class="text-pink-600">Фамилия:</strong> {{ viewTeacherData.last_name }}</p>
                    <p><strong class="text-pink-600">Email:</strong> {{ viewTeacherData.email }}</p>
                    <p><strong class="text-pink-600">Телефон:</strong> {{ viewTeacherData.phone || 'Не указан' }}</p>
                    <p><strong class="text-pink-600">Дата найма:</strong> {{ formatDate(viewTeacherData.hire_date) }}</p>
                    <p><strong class="text-pink-600">Кафедра:</strong> {{ viewTeacherData.department }}</p>
                    <p><strong class="text-pink-600">Место работы:</strong> {{ viewTeacherData.office_location || 'Не указано' }}</p>
                    <p><strong class="text-pink-600">Квалификация:</strong> {{ viewTeacherData.qualification || 'Не указана' }}</p>
                ...
                <button @click="showView = false" class="mt-4 bg-pink-100 hover:bg-pink-200 text-pink-700 px-4 py-2 rounded-lg transition-colors mx-auto block">
                    Закрыть
                </button>
            </div>
        </div>
...
<style scoped>
.shadow-pink-300\/50 {
    box-shadow: 0 4px 6px -1px rgba(249, 168, 212, 0.5), 0 2px 4px -1px rgba(249, 168, 212, 0.3);
}
</style>
